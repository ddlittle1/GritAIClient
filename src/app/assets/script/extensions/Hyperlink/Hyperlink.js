  ...
  var HyperlinkTool = function (viewer, config) {
    ...
    var _showTooltip = function (x, y, intersectPoint) {
      if (!_tooltip) {
        ...
        _tooltip.textContent = av.i18n.translate(
          "Ctrl + Click to preview the link"
        );
      }
    ...
    this.handleSingleClick = function (event, button) {
      ...
      if (viewer.model.is2d()) {
        var viewportCoords = viewer.impl.clientToViewport(
          event.canvasX,
          event.canvasY
        );
        var intersectPoint =
          viewer.impl.intersectGroundViewport(viewportCoords);

        for (var index in _linkCache2d) {
          if (!_linkCache2d.hasOwnProperty(index)) continue;
          ...
          if (
            ...
            if (hyperLink.node) {
              ...
              this.panel = new av.Extensions.Hyperlink.HyperlinkPanel(
                self,
                viewer,
                position,
                intersectPoint,
    ...
    this.loadHyperlinks = function () {
      ...
      viewer.model.getBulkProperties(
        uniqueDbIds,
        { propFilter: ["hyperlink"] },
        this._onBulkProperties
      );
    };
    ...
    this._onBulkProperties = function (results) {
      ...
      for (var i = 0, len = results.length; i < len; ++i) {
        ...
        var candidates = docNode
          ? docNode.search({ viewableID: propValue })
          : null;
    ...
    this.getThumbnail = function (dbId, item, callback) {
      ...
      avp.ViewingService.getThumbnail(
        avp.initLoadContext(),
        urlpath,
        onSuccess,
        null,
    ...
    this.addRectangle = function (index, minX, minY, maxX, maxY) {
      ...
      var mesh = (_linkCache2d[index].bBoxMesh = new THREE.Mesh(
        geom,
        _bBoxMaterial,
        true
      ));
...
(function () {
  ...
  var HyperlinkPanel = function (
    ...
    this.viewButton.addEventListener("click", function () {
      ...
      if (item) {
        viewer.HyperlinkBack = new aveh.HyperlinkBack(
          hyperlinkTool,
          viewer,
          currItem
        );
