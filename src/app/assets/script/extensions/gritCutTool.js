(function () {
  ...
  function GritCutToolExtension(viewer, options) {
    window.addEventListener("ROUTE_CHANGE", (e) => {
      disableCut(true);
      if (
        e.detail.endsWith("models") ||
        e.detail.endsWith("master-schedule") ||
        e.detail.endsWith("planner")
      ) {
        if (viewer.toolbar)
    ...
    function side(vertex, plane) {
      const inner =
        vertex[0] * plane[0] + vertex[1] * plane[1] + vertex[2] * plane[2];
    ...
    function intersect(inside, outside, plane) {
      ...
      const best =
        Math.abs(inner) < 0.0001
          ? 1
          : (plane[3] -
              inside[0] * plane[0] -
              inside[1] * plane[1] -
              inside[2] * plane[2]) /
            (dir[0] * plane[0] + dir[1] * plane[1] + dir[2] * plane[2]);
      return [
        inside[0] + best * dir[0],
        inside[1] + best * dir[1],
        inside[2] + best * dir[2],
    ...
    function SplitMesh(mesh, plane) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        for (let l = 0; l < mesh.fragments.length; l++) {
          ...
          for (
            let j = vertexIndex;
            j < vertexIndex + mesh.fragments[l] * 3;
            j += 3
          ) {
            let flag = 1;
            const vertex = [
          ...
          for (let j = 0; j < mesh.copies.length; j++) {
            ...
            if (
              ci >= vertexIndex / 3 &&
              ci < vertexIndex / 3 + mesh.fragments[l]
            ) {
              if (insideMap[ci] != null) {
                ...
                outsideMap[mesh.vertices.length / 3 + j] =
                  outsideVertices.length;
          ...
          for (let j = 0; j < mesh.triangles.length; j += 3) {
            ...
            if (inCount === 3) {
              ...
              const int1 = intersect(
                insideVertices[insideMap[index[(k + 1) % 3]]],
                outside,
                plane
              );
              const int2 = intersect(
                insideVertices[insideMap[index[(k + 2) % 3]]],
                outside,
                plane
              );
              ...
              const int1 = intersect(
                inside,
                outsideVertices[outsideMap[index[(k + 1) % 3]]],
                plane
              );
              const int2 = intersect(
                inside,
                outsideVertices[outsideMap[index[(k + 2) % 3]]],
                plane
              );
              triangles.push(insideMap[index[k]]);
              triangles.push(insideVertices.length);
          ...
          newMesh.fragments.push(insideVertices.length);
          triangles.forEach((t) =>
            newMesh.triangles.push(t + newMesh.vertices.length / 3)
          );
    ...
    var getGeometry = function (dbId, model) {
      ...
      svf.fragments.fragId2dbId.forEach((id, fragId) => {
        if (dbId == id) {
          ...
          var mat =
            svf.materials.materials[svf.fragments.materials[fragId]]
              .materials[0].properties.colors.generic_diffuse;
          var f = getFragment(
            geom,
            mat ? mat.values[0] : null,
            [0, 0, 0],
            false,
            svf.fragments.transforms.slice(fragId * 12, fragId * 12 + 12),
            1
          );
    ...
    var getFragment = function (
      ...
      const uvOffset = geometry.attributes.uv
        ? geometry.attributes.uv.itemOffset
        : null;
    ...
    var getPlane = function () {
      ...
      var vector = new THREE.Vector3(1, 0, 0).applyQuaternion(
        camera.quaternion
      );
    ...
    var previewCut = function (save) {
      ...
      cutMeshes.forEach((data) => {
        ...
        for (let i = 0; i < data.triangles.length; i += 3) {
          ...
          const v1 =
            t1 * 3 >= data.vertices.length
              ? data.copies[t1 - data.vertices.length / 3] * 3
              : t1 * 3;
          const v2 =
            t2 * 3 >= data.vertices.length
              ? data.copies[t2 - data.vertices.length / 3] * 3
              : t2 * 3;
          const v3 =
            t3 * 3 >= data.vertices.length
              ? data.copies[t3 - data.vertices.length / 3] * 3
              : t3 * 3;
        ...
        const colorHex =
          Math.floor(color.r * 255) * 0x10000 +
          Math.floor(color.g * 255) * 0x100 +
          Math.floor(color.b * 255);
        const material = new THREE.MeshPhongMaterial({ color: colorHex });
        viewer.impl
          .matman()
          .addMaterial("SplitMesh-" + cutObjects.length, material, true);
    ...
    var saveCut = function () {
      ...
      var detail = {
        meshes: cutMeshes,
        originalObjectId: cutHit.originalDbId,
        objectId: cutHit.dbId,
        modelId: cutHit.model.id,
        planes: planes,
      };
      disableCut(true);
    ...
    var disableCut = function (force) {
      ...
      if (planes.length > 0 && !force) {
        ...
        window.dispatchEvent(
          new CustomEvent("VIEWER_CUT", { detail: { disabled: true } })
        );
      }
    };
          ...
          models.forEach((model) => {
            ...
            fragList.forEach((num) => {
              if (!map[num]) {
                map[num] = true;
                if (
                  !model.visibilityManager.getFragmentMap().isNodeHidden(num)
                ) {
                  array.push(num);
    ...
    this.init = function (
      ...
      if (!initialized) {
        ...
        if (
          loadedRoute.endsWith("models") ||
          loadedRoute.endsWith("master-schedule") ||
          loadedRoute.endsWith("planner")
        ) {
          viewer.toolbar.getControl("navTools").addControl(cutButton);
        }
      }
