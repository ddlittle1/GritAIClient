(function(){
  ...
  window.addEventListener('ROUTE_CHANGE', (e) => { proto.setCurrentRouteForMenu(e.detail)});
  
  window.addEventListener('SELECTING_PREREQS', (e) => { proto.setSelectingPrereqs(e.detail)});

  window.addEventListener('SELECTED_WORK_PACKAGE', (e) => { proto.setWorkPackageSelected(e.detail)});
  ...
  GritContextMenuExternalExtension.prototype = Object.create(Autodesk.Viewing.Extension.prototype)
  GritContextMenuExternalExtension.prototype.constructor = GritContextMenuExternalExtension
...
class GritContextMenu extends Autodesk.Viewing.Extensions.ViewerObjectContextMenu {
  constructor( viewer ) {
    super( viewer );
  }
  ...
  async buildMenu(event) {
    ...
    const selectingPrereqs = this.viewer.loadedExtensions.GritContextMenu.selectingPrereqs;
    const workPackageSelected = this.viewer.loadedExtensions.GritContextMenu.workPackageSelected;
    const TranslationService = this.viewer.loadedExtensions.GritContextMenu.TranslationService;
    ...
    if (similarObjectRoutes.filter(r => curRoute.includes(r)).length > 0) {
      ...
      if (result) {
        menu.push({
          ...
          target: () => {
            var contextMenuEvent = new CustomEvent('VIEWER_SHOW_SIMILAR', {detail: result});
    ...
    if (workPackageSelected && addUnmodeledRoutes.filter(r => curRoute.includes(r)).length > 0) {
      menu.push({
        title: TranslationService.translate('add_unmodeled_task'),
        target: () => {
          detailData.event = 'ADD_UNMODELED';
          detailData.data = {};
          var contextMenuEvent = new CustomEvent('GRIT_CONTEXT_MENU', {detail: detailData});
          window.dispatchEvent(contextMenuEvent);
        }
      });
    }
    if (selectingPrereqs && addActivityPrereqRoutes.filter(r => curRoute.includes(r)).length > 0) {
      menu.push({
