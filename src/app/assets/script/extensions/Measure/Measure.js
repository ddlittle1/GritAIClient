(function () {
  ...
  var DEFAULT_MEASUREMENT_TYPE =
    MeasureCommon.MeasurementTypes.MEASUREMENT_DISTANCE;
  ...
  MeasureExtension.prototype = Object.create(
  ...
  MeasureExtension.prototype.load = function () {
    ...
    this.hasUI =
      ...
      viewer instanceof Autodesk.Viewing.Private.GuiViewer3D;
    ...
    this.measureTool = new avem.MeasureTool(
      viewer,
      measureToolOptions,
      this.sharedMeasureConfig,
      this.snapper
    ...
    this.calibrationTool =
      new Autodesk.Viewing.Extensions.Measure.CalibrationTool(
        viewer,
        this.options,
        this.sharedMeasureConfig,
        this.snapper
    ...
    this.magnifyingGlass =
      new Autodesk.Viewing.Extensions.Measure.MagnifyingGlass(viewer);
    ...
    viewer.addEventListener(
      MeasureCommon.Events.MEASUREMENT_CHANGED_EVENT,
  ...
  function onModelLoaded(measureExt, model) {
    ...
    if (measureExt.viewer.toolbar) {
      ...
      measureExt.bindedOnToolbarCreated =
        measureExt.onToolbarCreated.bind(measureExt);
      measureExt.viewer.addEventListener(
        av.TOOLBAR_CREATED_EVENT,
        measureExt.bindedOnToolbarCreated
      );
    }
  ...
  function populateSharedMeasureConfig(measureExt, model) {
    ...
    measureExt.sharedMeasureConfig.calibrationFactor =
      measureExt.options.calibrationFactor;
  }
  ...
  MeasureExtension.prototype.getPrecisionOptions = function (isFractional) {
    ...
    else precisions = ["0", "0.1", "0.01", "0.001", "0.0001", "0.00001"];
  ...
  MeasureExtension.prototype.openCalibrationRequiredDialog = function (
    initiator
  ) {
    if (this.hasUI) {
      if (!this.CalibrationRequiredDialog) {
        this.CalibrationRequiredDialog =
          new Autodesk.Viewing.Extensions.Measure.CalibrationRequiredDialog(
            this,
            this.viewer,
    ...
    if (initiator === "measure") {
      ...
      this.activateInitiator = function () {
        this.viewer.dispatchEvent({
          type: Autodesk.Viewing.Extensions.Markups.Core
            .FINISHED_CALIBRATION_FOR_DIMENSION_EVENT,
        });
        this.activateInitiator = null;
      };
    }
  ...
  MeasureExtension.prototype.activate = function (mode) {
    ...
    switch (mode) {
      ...
      case "angle":
        this.enableMeasureTool(
          true,
          MeasureCommon.MeasurementTypes.MEASUREMENT_ANGLE
        );
        this.mode = "angle";
        break;
      case "area":
        if (!this.viewer.model.is2d()) {
          ...
          this.enableMeasureTool(
            true,
            MeasureCommon.MeasurementTypes.MEASUREMENT_AREA
          );
          this.mode = "area";
        }
        break;
   ...
   * @private
   */
  MeasureExtension.prototype.enableMeasureTool = function (
    enable,
    measurementType
  ) {
    if (
    ...
    if (enable && !isActive) {
      ...
      if (
        !this.forceCalibrate ||
        (this.forceCalibrate && this.calibrationTool.isCalibrated()) ||
        ...
        this.viewer.addEventListener(
          avem.OPEN_TOOL_AFTER_CALIBRAION,
          function () {
            this.enableMeasureTool(true);
          }.bind(this),
          { once: true }
  ...
  MeasureExtension.prototype.changeMeasurementType = function (
    measurementType
  ) {
  ...
  MeasureExtension.prototype.enableCalibrationTool = function (enable) {
    ...
    if (enable && !isActive) {
      ...
      this.viewer.dispatchEvent({
        type: MeasureCommon.Events.UNITS_CALIBRATION_STARTS_EVENT,
      });

      if (this.measureToolbar) {
        this.measureToolbar.deactivateAllButtons();
        this.measureToolbar.activateButtonByType(
          MeasureCommon.MeasurementTypes.CALIBRATION
        );
      }
  ...
  MeasureExtension.prototype.enterMeasurementMode = function () {
    ...
    this.navigationControls = toolbar.getControl(
    ...
    this.measureControls = toolbar.getControl(
      Autodesk.Viewing.TOOLBAR.MEASURETOOLSID
    );
  ...
  MeasureExtension.prototype.exitMeasurementMode = function () {
    ...
    if (
      this.CalibrationRequiredDialog &&
      this.CalibrationRequiredDialog.isVisible()
    ) {
  ...
  MeasureExtension.prototype.createUI = function () {
    ...
    var modelTools = toolbar.getControl(av.TOOLBAR.MODELTOOLSID);
    this.measurementToolbarButton = new avu.Button(
    ...
    this.measureToolbar =
      new Autodesk.Viewing.Extensions.Measure.MeasureToolbar(this);
  ...
  MeasureExtension.prototype.calibrate = function (
    requestedUnits,
    requestedSize
  ) {
  ...
  MeasureExtension.prototype.calibrateByScale = function (
    requestedUnits,
    requestedScale
  ) {
  ...
  MeasureExtension.prototype.isCalibrationValid = function (
    requestedUnits,
    requestedSize
  ) {
    return this.calibrationTool.isCalibrationValid(
      requestedUnits,
      requestedSize
    );
  };
...
(function () {
  ...
  proto.changeCurrentMeasurement = function (measurement) {
    ...
    this.viewer.dispatchEvent({
      type: MeasureCommon.Events.MEASUREMENT_CHANGED_EVENT,
...
(function () {
  ...
  Autodesk.Viewing.Extensions.Measure.Indicator = function (
    viewer,
    measurement,
    tool
  ) {
  ...
  proto.initLabelMobileGestures = function (label, pointNumber) {
    ...
    this.hammer = new av.Hammer.Manager(label, {
      ...
      inputClass: av.isIE11
        ? av.Hammer.PointerEventInput
        : av.Hammer.TouchInput,
    });
  ...
  proto.changeEndpointOnEditStyle = function (endpointNumber, isEditing) {
    if (
      this.endpoints[endpointNumber] &&
      this.endpoints[endpointNumber].label
    ) {
      MeasureCommon.safeToggle(
        this.endpoints[endpointNumber].label,
        "on-edit",
        isEditing
      );
    ...
    if (this.viewer.model && !this.viewer.model.is2d()) {
      this.visibleLabels.forEach(
        function (label) {
          ...
          if (
            (label.classList.contains("visible") && !show) ||
            (!label.classList.contains("visible") && show)
          ) {
  ...
  proto.renderEndpointGeometry = function (pickNumber) {
    var geometry = MeasureCommon.getSnapResultPosition(
      this.measurement.getPick(pickNumber),
      this.viewer
    );
  ...
  proto.drawEdgeAsCylinder = function (
    geom,
    material,
    linewidth,
    type,
  ...
  proto.drawDashedLine = function (
    ...
    line = this.drawEdgeAsCylinder(
      geometry,
      material,
      width,
      1,
      this.getNewCylinderGeometry()
    );
  ...
  proto.setCylinderScale = function (cylinderMesh, p1, p2) {
    ...
    if (p1 && p2) {
      var point = MeasureCommon.nearestPointInPointToSegment(
  ...
  proto.setScale = function (point) {
    ...
    var distance = camera.isPerspective
      ? p.sub(position).length()
  ...
  proto.alignLabelWithLine = function (label, p1, p2, offset, viewer) {
    ...
    label.style.transform =
      "rotate(" + angle + "deg) translate(0px, " + offset + "px)";
  };
  ...
  proto.destroy = function () {
    ...
    for (var name in this.endpoints) {
      if (this.endpoints.hasOwnProperty(name)) {
        ...
        if (endPoint.label) {
          endPoint.label.removeEventListener(
            "mousewheel",
            this.viewer.toolController.mousewheel
          );
  ...
  proto.renderRubberband = function (picks) {
    switch (this.measurement.measurementType) {
      case MeasureCommon.MeasurementTypes.MEASUREMENT_DISTANCE:
        ...
        var start = MeasureCommon.getSnapResultPosition(
          previewsPick,
          this.viewer
        );
...
(function () {
  ...
  function MeasureToolIndicator(viewer, measurement, measureTool) {
    ...
    this.lines = {
      xyz: {
        axis: false,
        material: createLineMaterial("005BCE"),
        className:
          "adsk-icon-axis-delta" +
          "-xyz measure-label-axis-delta" +
          " measure-label-axis-xyz",
      },
      x: {
        axis: true,
        material: createLineMaterial("F12C2C"),
        className:
          "adsk-icon-axis-delta" +
      ...
      y: {
        axis: true,
        material: createLineMaterial("0BB80B"),
        className:
          "adsk-icon-axis-delta" +
      ...
      z: {
        axis: true,
        material: createLineMaterial("2C2CF1"),
        className:
          "adsk-icon-axis-delta" +
  ...
  proto.init = function () {
    ...
    this.viewer.addEventListener(
      ...
      this.onCameraChangeBinded
    );

    this.handleButtonUpBinded = this.measureTool.handleButtonUp.bind(
      this.measureTool
    );
  ...
  proto.createEndpoint = function (index) {
    ...
    var currLabel = (this.endpoints[index].label =
      this.createSnapResultLabel(index));
    this.viewer.container.appendChild(currLabel);
  };

  proto.updateLabelsPosition = function () {
    var point, xy, label, key;
      ...
      if (this.lines.hasOwnProperty(name)) {
        ...
        if (label && item.p1 && item.p2 && isVisible(label)) {
          ...
          if (this.viewer.model && this.viewer.model.is2d()) {
            ...
            if (
              label.clientWidth >=
              p1Projected.distanceTo(p2Projected) -
                this.endpoints[1].label.clientWidth
            ) {
            ...
            if (item !== this.lines.xyz) {
              ...
              xyzDirection
                .subVectors(this.lines.xyz.p1, this.lines.xyz.p2)
                .normalize();
              itemDirection.subVectors(item.p1, item.p2).normalize();
              var normal = xyzDirection
                .cross(this.viewer.navigation.getEyeVector())
                .normalize();
    ...
    if (this.angleLabel) {
      ...
      if (
        ...
        point = {
          x: (this.angleLabel.p1.x + this.angleLabel.p2.x) / 2,
          y: (this.angleLabel.p1.y + this.angleLabel.p2.y) / 2,
          z: (this.angleLabel.p1.z + this.angleLabel.p2.z) / 2,
        };
    ...
    if (this.areaLabel) {
      ...
      if (label && this.areaLabel.p1 && this.areaLabel.p2 && isVisible(label)) {
        point = {
          x: (this.areaLabel.p1.x + this.areaLabel.p2.x) / 2,
          y: (this.areaLabel.p1.y + this.areaLabel.p2.y) / 2,
          z: (this.areaLabel.p1.z + this.areaLabel.p2.z) / 2,
        };
        xy = MeasureCommon.project(point, this.viewer);
        label.style.top = xy.y - Math.floor(label.clientHeight / 2) + "px";
        label.style.left = xy.x - Math.floor(label.clientWidth / 2) + "px";
    ...
    if (this.viewer.model && !this.viewer.model.is2d()) {
      ...
      for (i = 0; i < this.labels.length; i++) {
        backupPositions.push({
          left: this.labels[i].style.left,
          top: this.labels[i].style.top,
        });
      }
      ...
      for (i = 0; i < this.labels.length && !needToStackLabels; i++) {
        ...
        needToStackLabels = this.labelsOverlapDetection(
          currentLabel,
          this.labels
        );
      }
  ...
  function moveLeft(currentLabel, currentRect, otherRect) {
    currentLabel.style.left =
      parseInt(currentLabel.style.left, 10) -
  ...
  function moveDown(currentLabel, currentRect, otherRect) {
    currentLabel.style.top =
      parseInt(currentLabel.style.top, 10) +
  ...
  proto.stackLabels = function (labels) {
    ...
    for (var i = 1; i < labels.length; i++) {
      if (labels[i].causeStacking) {
        ...
        var top =
          labels[i] === topLabel ? topLabel.style.top : labels[i - 1].style.top;
  ...
  proto.drawXYZLine = function (item) {
    ...
    var normal = direction
      .clone()
      .cross(self.viewer.navigation.getEyeVector())
      .normalize();
    var point = MeasureCommon.nearestPointInPointToSegment(
      self.viewer.navigation.getPosition(),
      p1,
      p2
    );
    ...
    function drawTip(p) {
      ...
      var line = self.drawLineAsCylinder(
        geometry,
        item.material,
        _tipWidth,
        self.overlayName
  ...
  proto.redrawDashedLine = function () {
    if (!this.dashedLine.p1 || !this.dashedLine.p2) return;

    this.viewer.impl.removeMultipleOverlays(
      this.overlayName,
    ...
    this.dashedLine.line = this.drawDashedLine(
      this.dashedLine.p2,
      this.dashedLine.p1,
      dashSize,
      gapSize,
      this.lines.xyz.material,
      _dashedSegmentWidth,
      this.overlayName
    );
  ...
  proto.renderAreaMeasurement = function (picks) {
    ...
    if (count > 2) {
      ...
      if (
        MeasureCommon.isPolygonSimple(points) &&
        this.measurement.area !== 0
      ) {
  ...
  proto.drawAngle = function (p, ep1, ep2, n, angle, midPoint) {
    ...
    if (
      ((a13 <= smallNum && a13 >= -smallNum) ||
        (a14 <= smallNum && a14 >= -smallNum)) &&
      ((a23 <= smallNum && a23 >= -smallNum) ||
        (a24 <= smallNum && a24 >= -smallNum))
    ) {
      ra = 0;
    }
    // The arc needs to be rotated 180 degree to the right place
    else if (
      ((a13 <= Math.PI + smallNum && a13 >= Math.PI - smallNum) ||
        (a14 <= Math.PI + smallNum && a14 >= Math.PI - smallNum)) &&
      ((a23 <= Math.PI + smallNum && a23 >= Math.PI - smallNum) ||
        (a24 <= Math.PI + smallNum && a24 >= Math.PI - smallNum))
    ) {
      ra = Math.PI;
    }
    // The arc needs to be rotated a13 radian
    else if (
      (a13 <= a23 + smallNum && a13 >= a23 - smallNum) ||
      (a13 <= a24 + smallNum && a13 >= a24 - smallNum)
    ) {
      ra = a13;
    }
    ...
    var outlineGeometry = new THREE.CircleGeometry(
      radius,
      segments,
      0,
      (angle * Math.PI) / 180
    ...
    this.angleOutline = this.drawEdgeAsCylinder(
      outlineGeometry,
      this.lines.xyz.material,
      _angleArcWidth,
      0,
    ...
    midPoint.copy(
      arc.geometry.vertices[Math.round(arc.geometry.vertices.length / 2) - 1]
    );
  ...
  proto.renderDistanceMeasurement = function (p1, p2) {
    ...
    function updateLine(item, x1, y1, z1, x2, y2, z2, showAxis) {
      ...
      if (
        ((self.isLeaflet && item !== self.lines.z) ||
          p1.distanceTo(p2) >= MeasureCommon.EPSILON) &&
        showAxis
      ) {
        item.p1 = p1;
        item.p2 = p2;
        ...
        if (item === self.lines.xyz) {
          ...
          line = item.line = self.drawLineSegment(
            p1,
            p2,
            _axisLineWidth,
            item.material
    ...
    function displayAxis(p1, p2) {
      ...
      return (
        !MeasureCommon.isParallel(self.tmpVector, self.xAxis) &&
        !MeasureCommon.isParallel(self.tmpVector, self.yAxis) &&
        !MeasureCommon.isParallel(self.tmpVector, self.zAxis)
      );
    }
  ...
  proto.updateDistance = function () {
    ...
    Object.keys(this.lines).forEach(
      function (name) {
        setWidth(this.lines[name].label, "");
      }.bind(this)
    );

    setDeltaMeasurementLabelText(
      this.lines.x.label,
      "~ " + this.measureTool.getDistanceX(this.measurement)
    );
    setDeltaMeasurementLabelText(
      this.lines.y.label,
      "~ " + this.measureTool.getDistanceY(this.measurement)
    );
    setDeltaMeasurementLabelText(
      this.lines.z.label,
      "~ " + this.measureTool.getDistanceZ(this.measurement)
    );
    setValueMeasurementLabelText(
      this.lines.xyz.label,
      "~ " + this.measureTool.getDistanceXYZ(this.measurement)
    );
      ...
      setTimeout(
        function () {
          var maxWidth = Math.max(
            getWidth(this.lines.x.label),
            getWidth(this.lines.y.label),
            getWidth(this.lines.z.label),
            getWidth(this.lines.xyz.label)
          );
          Object.keys(this.lines).forEach(
            function (name) {
  ...
  proto.updateAngle = function () {
    setValueMeasurementLabelText(
      this.angleLabel.label,
      "~ " + this.measureTool.getAngle(this.measurement)
    );
  };
  ...
  proto.updateArea = function () {
    setValueMeasurementLabelText(
      ...
      "~ " + this.measureTool.getArea(this.measurement)
    );
  };
  ...
  proto.destroy = function () {
    ...
    for (name in this.lines) {
      if (this.lines.hasOwnProperty(name)) {
        ...
        if (item.label) {
          if (name === "xyz") {
            item.label.addEventListener(
              "mousewheel",
              this.viewer.toolController.mousewheel
            );
            item.label.removeEventListener(
              "click",
              this.onSelectionAreaClickedBinded
            );
          }
    ...
    if (this.angleLabel && this.angleLabel.label) {
      ...
      this.angleLabel.label.removeEventListener(
        "mousewheel",
        this.viewer.toolController.mousewheel
      );
      this.angleLabel.label.removeEventListener(
    ...
    if (this.areaLabel && this.areaLabel.label) {
      ...
      this.areaLabel.label.removeEventListener(
        "mousewheel",
        this.viewer.toolController.mousewheel
      );
      this.areaLabel.label.removeEventListener(
  ...
  proto.setNoTopology = function () {
    this.topologyStatus = TOPOLOGY_NOT_AVAILABLE;
    if (this.lines.xyz.label) {
      setVisibilityMeasurementLabelSpinner(
        this.lines.xyz.label,
        false,
        this.topologyStatus
      );
  ...
  // Receives an object created with createMeasurementLabel()
  function setVisibilityMeasurementLabelSpinner(
    label,
    isVisible,
    topologyStatus
  ) {
    ...
    div &&
      (div.style.display =
        isVisible && topologyStatus === TOPOLOGY_FETCHING
          ? "inline-block"
          : "none");
  }
  ...
  proto.createSelectionArea = function () {
    ...
    selectionArea.addEventListener(
      "mousewheel",
      this.viewer.toolController.mousewheel
    );
    ...
    this.segments.forEach(
      function (item) {
        if (item.p1 && item.p2) {
          ...
          selectionArea.style.transformOrigin =
            "0px " + _selectorAreaSize / 2 + "px";
        }
      }.bind(this)
    );
  ...
  proto.clearSelectionAreas = function () {
    this.segments.forEach(
      function (item) {
        if (item.selectionArea) {
          item.selectionArea.removeEventListener(
            "mousewheel",
            this.viewer.toolController.mousewheel
          );
          item.selectionArea.removeEventListener(
          ...
          var element = document.getElementById(
            "measurement-selection-area-" + this.measurement.id
          );
          if (element) {
  ...
  Autodesk.Viewing.Extensions.Measure.MeasureToolIndicator =
...
(function () {
  ...
  //
  //
  Autodesk.Viewing.Extensions.Measure.MeasureTool = function (
    viewer,
    options,
    sharedMeasureConfig,
    snapper
    ...
    function getActivePick() {
      if (!_currentMeasurement) return null;
      ...
      if (_activePoint === 0) {
        ...
        _activePoint ===
        _currentMeasurement.getMaxNumberOfPicks() + 1
      ) {
    ...
    function hasPreviousPick() {
      if (!_currentMeasurement) return false;
    ...
    this.areAllPicksSet = function () {
      return (
        _currentMeasurement &&
        _activePoint === _currentMeasurement.getMaxNumberOfPicks() + 1
      );
    };
    ...
    this.startNewMeasurement = function () {
      _currentMeasurement =
        _measurementsManager.createMeasurement(_measurementType);
      _currentMeasurement.attachIndicator(
        _viewer,
        this,
        avem.MeasureToolIndicator
      );
    ...
    this.activate = function () {
      ...
      _viewer.addEventListener(
        MeasureCommon.Events.MEASUREMENT_CHANGED_EVENT,
        this.onMeasurementChangedBinded
      );
    ...
    this.deactivate = function () {
      ...
      while (Object.keys(_measurementsManager.measurementsList).length > 0) {
        _currentMeasurement =
          _measurementsManager.measurementsList[
            Object.keys(_measurementsManager.measurementsList)[0]
          ];
    ...
    this.getDistanceAux = function (measurementDistance) {
      if (_viewer.model) {
        var d = Autodesk.Viewing.Private.convertUnits(
          _viewer.model.getUnitString(),
          _sharedMeasureConfig.units,
        ...
        return Autodesk.Viewing.Private.formatValueWithUnits(
          ...
          _sharedMeasureConfig.units,
          3,
          _sharedMeasureConfig.precision
        );
      }
    };
    ...
    this.getArea = function (measurement) {
      if (_viewer.model) {
        ...
        var area = Autodesk.Viewing.Private.convertUnits(
    ...
    function snapToFirstPick(currentPick, forceSnap) {
      if (
        _currentMeasurement.hasPick(1) &&
        _activePoint > 3 &&
        !_currentMeasurement.closedArea
      ) {
        var firstPick = _currentMeasurement.getPick(1);
        var firstPickPoint = MeasureCommon.getSnapResultPosition(
          firstPick,
          _viewer
        );
        var firstPickPosition = MeasureCommon.project(firstPickPoint, viewer);
        var currentPickPoint = MeasureCommon.getSnapResultPosition(
          currentPick,
          _viewer
        );
        var currentPickPosition = MeasureCommon.project(
          currentPickPoint,
          viewer
        );
        ...
        if (
          forceSnap ||
          currentPickPosition.distanceTo(firstPickPosition) <
            _closeAreaSnapRange
        ) {
    ...
    function render(showResult) {
      var hasResult = _currentMeasurement.computeResult(
        _currentMeasurement.picks,
        _viewer,
        _snapper
      );

      _currentMeasurement.indicator.render(
        _currentMeasurement.picks,
        _consumeSingleClick || !!showResult
      );
    ...
    this.selectMeasurementById = function (measurementId) {
      if (!_currentMeasurement) {
        _currentMeasurement =
          _measurementsManager.selectMeasurementById(measurementId);
      }
    ...
    this.editEndpoint = function (event, endpointNumber, measurementId) {
      if (
        _currentMeasurement.id === measurementId &&
        _activePoint === endpointNumber
      ) {
        _currentMeasurement.indicator.changeEndpointOnEditStyle(
          endpointNumber,
          false
        );
        this.undoEditEndpoint();
    ...
    this.updateViewportId = function (clear) {
      if (_viewer.model && _viewer.model.is2d()) {
        if (clear || isNoPicksSet()) {
          ...
          var viewport =
            getPreviousPick().viewportIndex2d ||
          ...
          if (_snapper) _snapper.setViewportId(viewport);
        }
      }
    };
    ...
    this.correctPickPosition = function () {
      ...
      if (hasPreviousPick()) {
        ...
        var corrected = MeasureCommon.correctPerpendicularPicks(
          passive,
          active,
          viewer,
          _snapper
        ...
        if (!corrected) {
          ...
          if (id !== active.id && _currentMeasurement.hasPick(id)) {
            ...
            MeasureCommon.correctPerpendicularPicks(
              nextPick,
              active,
              viewer,
              _snapper
        ...
        if (
          _currentMeasurement.measurementType ==
          MeasureCommon.MeasurementTypes.MEASUREMENT_AREA
        ) {
    ...
    this._handleMouseEvent = function (event) {
      ...
      if (_snapper.isSnapped()) {
        ...
        if (event && _viewer.model.is2d()) {
          ...
          if (x && y) {
            _cursorPosition = MeasureCommon.inverseProject(
              { x: x, y: y },
              _viewer
            );
          }
    ...
    this._doConsumeSingleClick = function () {
      ...
      if (
        ...
        if (
          length >= 3 &&
          _currentMeasurement.hasEqualPicks(firstPick, lastPick)
        ) {
          lastPick.clear();
    ...
    this.handleButtonDown = function (event, button) {
      if (av.isMobileDevice()) return false;
    ...
    this.restoreMouseListeners = function () {
      ...
      _viewer.toolController
        .getTool("gestures")
        .controller.enableMouseButtons(true);
    };
    ...
    this.handleDoubleTap = function (event) {
      if (
        ...
        _currentMeasurement.measurementType ===
          MeasureCommon.MeasurementTypes.MEASUREMENT_AREA &&
        _currentMeasurement.countPicks() > 2
      ) {
...
(function () {
  ...
  function CalibrationToolIndicator(viewer, measurement, calibrationTool) {
    Autodesk.Viewing.Extensions.Measure.Indicator.call(
      this,
      viewer,
      measurement,
      calibrationTool
  ...
  CalibrationToolIndicator.prototype = Object.create(
    Autodesk.Viewing.Extensions.Measure.Indicator.prototype
  );
  ...
  proto.init = function () {
    ...
    if (!this.calibrationLabel) {
      ...
      this.calibrationLabel.addEventListener(
        "mousewheel",
        this.viewer.toolController.mousewheel
      );
    ...
    this.handleButtonUpBinded = this.calibrationTool.handleButtonUp.bind(
      this.calibrationTool
    );
    window.addEventListener("mouseup", this.handleButtonUpBinded);
  ...
  proto.destroy = function () {
    ...
    if (this.calibrationLabel) {
      this.calibrationLabel.removeEventListener(
        "mousewheel",
        this.viewer.toolController.mousewheel
      );
  ...
  proto.updateLabelsPosition = function () {
    for (var i = 1; i <= Object.keys(this.endpoints).length; i++) {
      if (this.endpoints[i].position) {
        ...
        var pos = MeasureCommon.project(
          this.endpoints[i].position,
          this.viewer,
          kEndpointOffset
        );
  ...
  proto.renderCalibrationLabel = function () {
    if (this.showMeasureResult && this.calibrationLabel && this.p1 && this.p2) {
      ...
      this.calibrationLabel.style.top =
        this.labelPosition.y -
        Math.floor(this.calibrationLabel.clientHeight / 2) +
        "px";
      this.calibrationLabel.style.left =
        this.labelPosition.x -
        Math.floor(this.calibrationLabel.clientWidth / 2) +
        "px";
      ...
      if (this.viewer.model.is2d()) {
        this.alignLabelWithLine(
          ...
          this.p1,
          this.p2,
          this.calibrationLabel.clientHeight,
          this.viewer
        );
      }
    }
  ...
  proto.drawMeasurementLineTip = function (point, direction, normal, flip) {
    ...
    var tipMaterial =
      this.snapper.isSnapped() && !this.showMeasureResult
        ? this.rubberbandSnappedMaterial
        : this.rubberbandTipMaterial;

    // black tip
    tmpVec.addVectors(
      point,
      normal.clone().multiplyScalar(kCrossLength * p1Scale)
    );
    geometry.vertices[0] = tmpVec.clone();
    tmpVec.subVectors(
      point,
    ...
    tmpVec.subVectors(
      point,
      direction.clone().multiplyScalar(kCrossLength * p1Scale * flip)
    );
    geometry.vertices[1] = tmpVec.clone();
    this.drawLineAsCylinder(
      geometry,
    ...
    tmpVec.addVectors(
      point,
      normal.clone().multiplyScalar(kTipXLength * p1Scale)
    );
    geometry.vertices[0] = tmpVec.clone();
    tmpVec.subVectors(
      point,
    ...
    this.drawLineAsCylinder(
      geometry,
      this.rubberbandDefaultMaterial,
      kTipXWidth,
      this.overlayName
    );
    ...
    tmpVec.addVectors(
      tmpVec,
      direction.clone().multiplyScalar(kTipXLength * p1Scale)
    );
    geometry.vertices[0] = tmpVec.clone();
    tmpVec.subVectors(
      point,
  ...
  proto.renderDistanceMeasurement = function (p1, p2) {
    ...
    var lineMaterial =
      (Math.abs(p1.x - p2.x) <= 0.1 ||
        ...
        this.snapper.getSnapResult().isPerpendicular) &&
  ...
  Autodesk.Viewing.Extensions.Measure.CalibrationToolIndicator =
    CalibrationToolIndicator;
})();
...
//
//
Autodesk.Viewing.Extensions.Measure.CalibrationTool = function (
  viewer,
  options,
  sharedMeasureConfig,
  snapper
  ...
  var _priority = 50;
  var _viewer = viewer;
  var _measurement = new MeasureCommon.Measurement(
    MeasureCommon.MeasurementTypes.MEASUREMENT_DISTANCE
  );
  var _options = options || {};
  ...
  this.register = function () {
    if (_hasUI && !_calibrationPanel) {
      _calibrationPanel =
        new Autodesk.Viewing.Extensions.Measure.CalibrationPanel(
          this,
          _viewer,
          "calibration-panel",
          "Calibration",
  ...
  this.activate = function () {
    ...
    if (!_measurement.indicator) {
      _measurement.attachIndicator(
        _viewer,
        this,
        avem.CalibrationToolIndicator
      );
    }
    ...
    if (_calibrationTaken && _selectedP1 && _selectedP2) {
      ...
      var parsedRequestedSize =
        Autodesk.Viewing.Private.UnitParser.parsePositiveNumber(
          _selectedSize,
          _selectedUnits
        );
      _measurement.indicator.updateLabelValue(
  ...
  this.calibrate = function (requestedUnits, requestedSize) {
    ...
    if (!isNaN(parsedRequestedSize)) {
      var currentSize = Autodesk.Viewing.Private.convertUnits(
        _viewer.model.getUnitString(),
        requestedUnits,
        1,
        _distance
      ...
      if (currentSize !== 0 && !isNaN(currentSize)) {
        ...
        var requestedPrecision =
          Autodesk.Viewing.Private.calculatePrecision(requestedSize);
        _sharedMeasureConfig.precision = Math.max(
          _sharedMeasureConfig.precision
            ? _sharedMeasureConfig.precision
            : defualtPrecision,
          requestedPrecision
  ...
  this.showPanel = function () {
    ...
    if (_calibrationPanel) {
      window.setTimeout(function () {
        _calibrationPanel.requestedSizeTextbox.focus();
      }, 0);
      _calibrationPanel.setVisible(true);
      _calibrationPanel.updatePanelPosition(
        _measurement.indicator.labelPosition,
        _measurement.indicator.p1,
        _measurement.indicator.p2,
        _measurement.indicator.calibrationLabel.clientHeight
      );
      ...
      _viewer.dispatchEvent({
        type: avem.OPEN_CALIBRATION_PANEL_EVENT,
        data: { size: _selectedSize, units: _selectedUnits },
      });
    }
  };
  ...
  this.updateViewportId = function (clear) {
    if (_viewer.model && _viewer.model.is2d()) {
      if (clear || isNoPicksSet()) {
        ...
        var viewport =
          getPreviousPick().viewportIndex2d || getActivePick().viewportIndex2d;
  ...
  this.render = function () {
    var hasResult = _measurement.computeResult(
      _measurement.picks,
      _viewer,
      _snapper
    );
    _measurement.indicator.render(
      _measurement.picks,
      _consumeSingleClick || _waitingForInput
    );
...
(function () {
  ...
  var CalibrationPanel = function (
    ...
    if (!options.heightAdjustment) options.heightAdjustment = 50;
    ...
    this.addEventListener(this.requestedSizeTextbox, "keypress", function (e) {
      ...
      requestedSize = [
        requestedSize.slice(0, cursorIndex),
        key,
        requestedSize.slice(cursorIndex),
      ].join("");
      ...
      if (
        !isPositiveNumber(requestedSize) ||
        (isSimple &&
          (!isSimpleDecimal(requestedSize) ||
            Autodesk.Viewing.Private.calculatePrecision(requestedSize) >
              self.calibrationTool.getMaxPrecision()))
      ) {
        e.preventDefault();
      }
    });
    ...
    this.units = [
      {
        ...
        units: "ft-and-fractional-in",
        matches: ["ft-and-fractional-in"],
        simpleInput: false,
      },
      {
        name: "Feet and decimal inches",
        units: "ft-and-decimal-in",
        matches: ["ft-and-decimal-in"],
        simpleInput: false,
      },
    ...
    this.unitList = new avp.OptionDropDown(
      "Unit type",
      this.tbody,
      unitNames,
      0,
    ...
    setCalibration.addEventListener(
      "click",
      function () {
        var index = self.unitList.selectedIndex;
        var requestedUnits = self.units[index].units;
        self.calibrationTool.calibrate(
          requestedUnits,
          self.requestedSizeTextbox.value
        );
      },
      false
    );
  ...
  CalibrationPanel.prototype.updateLabel = function () {
    ...
    var parsedNumber = Autodesk.Viewing.Private.UnitParser.parsePositiveNumber(
      size,
      requestedUnits
    );
    var text = Autodesk.Viewing.Private.formatValueWithUnits(
      parsedNumber,
      requestedUnits,
      3,
      Autodesk.Viewing.Private.calculatePrecision(size)
    );
  ...
  CalibrationPanel.prototype.updatePanelPosition = function (
    labelPosition,
    p1,
    p2,
    labelOffset
    ...
    if (!labelPosition || !p1 || !p2) {
      ...
      var rubberbandDirection = new THREE.Vector2()
        .copy(p1)
        .sub(p2)
        .normalize();
      ...
      if (p1.x < p2.x) {
        normal = new THREE.Vector2(
          rubberbandDirection.y,
          -rubberbandDirection.x
        );
      } else {
        normal = new THREE.Vector2(
          -rubberbandDirection.y,
      ...
      var offset =
        labelOffset + Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)) / 2;
      ...
      if (
        cornerX < 0 ||
        cornerX + width > this.viewer.canvas.clientWidth ||
        cornerY < 0 ||
        cornerY + height > this.viewer.canvas.clientHeight
      ) {
        cornerX = Math.floor((this.viewer.canvas.clientWidth - width) / 2);
        cornerY = Math.floor((this.viewer.canvas.clientHeight - height) / 2);
      }
  ...
  //
  //
  var CalibrationRequiredDialog = function (
    measureExt,
    viewer,
    id,
    title,
    ...
    calibrateNowButton.addEventListener(
      "click",
      function () {
  ...
  CalibrationRequiredDialog.prototype = Object.create(
    avu.DockingPanel.prototype
  );
...
Autodesk.Viewing.Extensions.Measure.MagnifyingGlass = function (viewer) {
  ...
  this.updateMagnifyingGlass = function () {
    if (_needsClear) {
      _magnifyingGlassCanvas &&
        _magnifyingGlassCanvas.classList.remove("visible");
      _needsClear = false;
    } else {
      ...
      if (!_magnifyingGlassCanvas) {
        ...
        _imageData = new ImageData(
          Math.ceil(normlizedDiameter / _zoom),
          Math.ceil(normlizedDiameter / _zoom)
        );
      ...
      gl.readPixels(
        x,
        _viewer.canvas.height - y - _imageData.height,
        _imageData.width,
        _imageData.height,
        gl.RGBA,
        gl.UNSIGNED_BYTE,
  ...
  this.requestUpdate = function () {
    if (
      !_viewer.hasEventListener(
        Autodesk.Viewing.RENDER_PRESENTED_EVENT,
        this.updateMagnifyingGlassBinded
      )
    ) {
      _viewer.addEventListener(
        Autodesk.Viewing.RENDER_PRESENTED_EVENT,
...
(function () {
  ...
  proto.init = function () {
    ...
    this.measureToolbar = new avu.ControlGroup(
      Autodesk.Viewing.TOOLBAR.MEASURETOOLSID
    );
    toolbar.addControl(this.measureToolbar, toolbarOptions);

    // Create a button for the measure simple distance.
    this.measureSimpleDistanceBtn = new avu.Button(
      "toolbar-measureTool-simple-distance"
    );
    ...
    this.measureSimpleDistanceBtn.onClick = function () {
      var enable =
        self.measureSimpleDistanceBtn.getState() !== avu.Button.State.ACTIVE;
      if (enable) {
    ...
    this.buttonsList[MeasureCommon.MeasurementTypes.MEASUREMENT_DISTANCE] =
      this.measureSimpleDistanceBtn;
    ...
    this.buttonsList[MeasureCommon.MeasurementTypes.MEASUREMENT_ANGLE] =
      this.measureAngleBtn;
    ...
    if (this.viewer.model && this.viewer.model.is2d()) {
      ...
      this.buttonsList[MeasureCommon.MeasurementTypes.MEASUREMENT_AREA] =
        this.measureAreaBtn;
    }
    ...
    this.calibrationToolBtn.onClick = function (e) {
      var enable =
        self.calibrationToolBtn.getState() !== avu.Button.State.ACTIVE;
      if (enable) {
    ...
    this.buttonsList[MeasureCommon.MeasurementTypes.CALIBRATION] =
      this.calibrationToolBtn;
    ...
    this.settingsControlPanel.classList.add(
      "docking-panel-container-solid-color-b"
    );
    ...
    this.units = [
      { name: "Unknown", units: "", matches: [""] }, // localized in OptionDropDown() call below
      {
        name: "Decimal feet",
        units: "decimal-ft",
        matches: ["ft", "decimal-ft"],
      }, // localized in OptionDropDown() call below
      {
        name: "Feet and fractional inches",
        units: "ft-and-fractional-in",
        matches: ["ft-and-fractional-in"],
    ...
    this.precisionList = new avp.OptionDropDown(
      "Precision",
      this.tbody,
      [],
      -1,
    ...
    this.precisionList.addEventListener("change", function (e) {
      ...
      avp.logger.track({
        category: "pref_changed",
        name: "measure/precision",
        value: index,
      });
    });

    this.isolate = new avp.OptionCheckbox(
      "Isolate measurement",
      this.tbody,
      false
    );
  ...
  proto.closeToolbar = function () {
    ...
    var modelTools = toolbar.getControl(av.TOOLBAR.MODELTOOLSID);
    modelTools.addControl(this.measureExtension.measurementToolbarButton, {
      index: this.measureExtension.measurementToolbarButton.index,
    });
  };
  ...
  proto.setupPrecision = function () {
    while (this.precisionList.dropdownElement.lastChild) {
      this.precisionList.dropdownElement.removeChild(
        this.precisionList.dropdownElement.lastChild
      );
    }
    ...
    if (
      selectedUnits === "ft-and-fractional-in" ||
  ...
  proto.disableUnitOption = function (index) {
    if (index != null) {
      ...
      if (av.isIE11) {
        ...
        this.unitList.dropdownElement.children[index].style.visibility =
          "hidden";
      }
    } else {
      // disable all options
  ...
  proto.updateSettingsPanel = function () {
    ...
    this.precisionList.dropdownElement.selectedIndex =
      this.measureTool.getPrecision();
    this.setupPrecision();
  };
