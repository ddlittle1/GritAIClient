...
collision.isIntersectionTriangleAABB = function (a, b, c, aabb) {
  ...
  if (
    Math.max(v0.x, v1.x, v2.x) < -extents.x ||
    Math.min(v0.x, v1.x, v2.x) > extents.x
  ) {
    return false;
  }
  // ... [-extents.y, extents.y] and [min(v0.y,v1.y,v2.y), max(v0.y,v1.y,v2.y)] do not overlap
  if (
    Math.max(v0.y, v1.y, v2.y) < -extents.y ||
    Math.min(v0.y, v1.y, v2.y) > extents.y
  ) {
    return false;
  }
  // ... [-extents.z, extents.z] and [min(v0.z,v1.z,v2.z), max(v0.z,v1.z,v2.z)] do not overlap
  if (
    Math.max(v0.z, v1.z, v2.z) < -extents.z ||
    Math.min(v0.z, v1.z, v2.z) > extents.z
  ) {
    return false;
  }
...
collision.isIntersectionSphereSphere = function (sphere1, sphere2) {
  ...
  return (
    sphere1.center.distanceToSquared(sphere2.center) <= radiusSum * radiusSum
  );
};
