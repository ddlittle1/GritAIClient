...
function SelectionWindow(viewer, options) {
    ...
    this.init = function (model, TranslationService) {
        ...
        orbitToolButtonDown = _viewer.toolController.getTool('orbit').handleButtonDown.bind(_viewer.toolController.getTool('orbit'));
        ...
        for (var step = 0; step < fragid2dbid.length; step++) {
            ...
            _boundingBoxInfo.push({ bbox: thisBox, dbId: fragid2dbid[step], model: model });
        }
        ...
        if (!initialized) {
            ...
            window.addEventListener('SELECTION_WINDOW_ENABLE', (evt) => { enable(evt.detail) });
    ...
    this.addMesh = function (mesh) {
      boundingBoxMap[mesh.model.id + ':' + mesh.dbId] = { bbox: mesh.box, dbId: mesh.dbId, model: mesh.model };
      _boundingBoxInfo.push(boundingBoxMap[mesh.model.id + ':' + mesh.dbId]);
    }

    this.removeMesh = function (mesh) {
      _boundingBoxInfo.splice(_boundingBoxInfo.indexOf(boundingBoxMap[mesh.model.id + ':' + mesh.dbId]), 1);
    }

    function setupToolbarButtons(TranslationService) {
        selectionWindowButton = new Autodesk.Viewing.UI.ComboButton('toolbar-selectionWindow');
        selectionWindowButton.onClick = (() => { toggleSelectionWindow(false); });
        selectionWindowButton.setToolTip(TranslationService.translate('selection_window_contains'));
        selectionWindowButton.icon.className = 'adsk-button-icon fas fa-compress fa-lg';

        selectionWindowSubButton = new Autodesk.Viewing.UI.Button('toolbar-selectionWindowSub');
        selectionWindowSubButton.onClick = (() => { swapSelectionWindowTool(true); });
        selectionWindowSubButton.setToolTip(TranslationService.translate('selection_window_intersects'));
        selectionWindowSubButton.icon.className = 'adsk-button-icon fas fa-expand fa-lg';
        selectionWindowButton.subMenu.addControl(selectionWindowSubButton);

        _viewer.toolbar.getControl('settingsTools').addControl(selectionWindowButton);
    }
    ...
    enable = function (enabled) {
        ...
        if (enabled) {
            ...
            _viewer.toolbar.getControl('settingsTools').removeControl(selectionWindowButton);
        }
    }
    ...
    function swapSelectionWindowTool(hideFlyOut) {
        ...
        if (contains) {
            ...
            selectionWindowButton.icon.className = 'adsk-button-icon fas fa-expand fa-lg';
            selectionWindowSubButton.icon.className = 'adsk-button-icon fas fa-compress fa-lg';
    ...
    function toggleSelectionWindow(swapping) {
        if (!active) {
            ...
            _viewer.toolController.getTool('orbit').handleButtonDown = function(e, b) {
                if (b !== 0) {
            ...
            selectionWindowButton.container.className = 'adsk-control adsk-button active';
            ...
            active = false;
            // Hide navigation tools
            _viewer.toolbar.getControl("navTools").container.style = "display: inline-block";

            // reset orbit tool button event
            _viewer.toolController.getTool('orbit').handleButtonDown = orbitToolButtonDown;

            // deactivate tool
            _viewer.toolController.deactivateTool('SelectionWindowTool');

            // unhighlight toolbar button
            selectionWindowButton.container.className = 'adsk-control adsk-button inactive';
    ...
    function onMouseMove(evt) {
        ...
        if (_running) {
            //get mouse points
            _mouseEnd.x = Math.min(Math.max(evt.clientX - offsetX, 1), _container.getBoundingClientRect().width);
            _mouseEnd.y = Math.min(Math.max(evt.clientY - offsetY, 0), _container.getBoundingClientRect().height - 7);
    ...
    function onMouseUp(evt) {
        ...
        if (_running) {
            ...
            if (_mouseStart.x - _mouseEnd.x + _mouseStart.y - _mouseEnd.y !== 0) {
                var ids = compute({ clientX: _mouseStart.x + offsetX, clientY: _mouseStart.y + offsetY },
                    { clientX: _mouseEnd.x + offsetX, clientY: _mouseEnd.y + offsetY },
                    !contains); // true:  partially intersected.  false: inside the area only
...
Autodesk.Viewing.theExtensionManager.registerExtension('SelectionWindow', SelectionWindow);
