...
Autodesk.Viewing.Extensions.WebVR.StereoRenderContext = function (
  _vrDisplay,
  camera,
  HUD,
  toolConfig,
  ...
  this.handleNavModeInput = (button, pose) => {
    switch (button) {
      ...
      case "L_y":
        heading.sub(
          new THREE.Vector3(0, 3, 0).applyQuaternion(
  ...
  this.modeWalkthrough = function (pose) {
    ...
    camera.target = camera.position
      .clone()
      .add(lookAtDir.clone().multiplyScalar(10));
  };
  ...
  function getStereoCamera(camera, frameData, leftEye, fixed, result) {
    ...
    result.projectionMatrix.elements = leftEye
...
Autodesk.Viewing.Extensions.WebVR.VRExtension.populateDefaultOptions =
  function (options) {
...
Autodesk.Viewing.Extensions.WebVR.VRExtension.prototype = Object.create(
  Autodesk.Viewing.Extension.prototype
);
Autodesk.Viewing.Extensions.WebVR.VRExtension.prototype.constructor =
  Autodesk.Viewing.Extensions.WebVR.VRExtension;
    ...
    navigator.getVRDisplays().then(function (displays) {
      if (displays.length > 0) {
        ...
        if (_vrDisplay.capabilities.canPresent) {
          ...
          // Register tool
          self.tool = new Autodesk.Viewing.Extensions.WebVR.VRTool(
            viewer,
            self,
            _vrDisplay
          );
      ...
      avp.logger.warn(
        "Attempted to load WebVR extension, but WebVR is not supported."
      );
    });
  });
...
Autodesk.Viewing.Extensions.WebVR.VRExtension.prototype.createUI = function (
  toolbar,
  vrTool
) {
  var self = this;
...
Autodesk.Viewing.Extensions.WebVR.VRExtension.prototype.unload = function () {
  ...
  if (toolbar && this.vrToolButton) {
    toolbar
      ...
      .removeControl(this.vrToolButton.getId());
  }
...
Autodesk.Viewing.Extensions.WebVR.Menu = function (_scale, viewer, cursor) {
  ...
  this.init = function (scene) {
    ...
    var icons = ["info_button.png", "deselect_button.png"];
...
Autodesk.Viewing.Extensions.WebVR.VRTool = function (
  viewer,
  vrExtension,
  _vrDisplay
) {
  ...
  var _HUD;
  var toolConfig = {
    webVR_cursor: avp.isExperimentalFlagEnabled(
      "webVR_cursor",
      vrExtension.options
    ),
    webVR_menu: avp.isExperimentalFlagEnabled(
      "webVR_menu",
      vrExtension.options
    ),
    webVR_orbitModel: avp.isExperimentalFlagEnabled(
      "webVR_orbitModel",
      vrExtension.options
    ),
  };
  ...
  this.activate = function (name) {
    ...
    _vrDisplay.modelScaleFactor = Math.max(
      Math.min(Math.min(boundsSize.x, boundsSize.y), boundsSize.z) / 10.0,
      0.0001
    );
    ...
    if (av.isMobileDevice()) {
      ...
      _canvas.addEventListener("dblclick", function () {
        if (_isActive) viewer.setActiveNavigationTool();
      });
    }
    if (toolConfig.webVR_cursor || toolConfig.webVR_menu) _HUD = this.initHUD();

    _stereoRenderContext =
      new Autodesk.Viewing.Extensions.WebVR.StereoRenderContext(
        _vrDisplay,
        _camera,
        _HUD,
        toolConfig,
  ...
  this.initHUD = function () {
    ...
    if (toolConfig.webVR_cursor) {
      ...
      cursor = new Autodesk.Viewing.Extensions.WebVR.Cursor(
        viewer.impl,
        viewer.autocam,
        _vrDisplay.modelScaleFactor,
        camYup,
        viewer
      );
      cursor.init(scene);
    ...
    if (toolConfig.webVR_menu) {
      ...
      var menu = new Autodesk.Viewing.Extensions.WebVR.Menu(
        _vrDisplay.modelScaleFactor,
        viewer,
        cursor
      );
  ...
  this.update = function (timeStamp, camOffset) {
    ...
    if (toolConfig.webVR_cursor) _HUD.cursor.update(cam.C, _HUD);
    if (toolConfig.webVR_menu) _HUD.menu.update(_HUD.camera);
  ...
  this.deactivate = function (name) {
    if (_vrDisplay) {
      if (_vrDisplay.isPresenting) _vrDisplay.exitPresent(); // Stops VR mode
    ...
    if (_HUD) _canvas.removeEventListener("click", _HUD.cursor.onClick);
  };
  ...
  var showHUD = function (isVisible) {
    ...
    if (tbar.length > 0) {
      // HACK: Assume only 1
      if (tbar[0] !== "") tbar[0].style.display = isVisible ? "block" : "none";
    }
  };
...
Autodesk.Viewing.Extensions.WebVR.Cursor = function (
  viewerImpl,
  autocam,
  _scale,
  USE_YUP,
  ...
  viewerImpl
    .matman()
    .addMaterial("ADN-Material-" + "common color material", selectedMat, true);
  viewerImpl
    .matman()
    .addMaterial("ADN-Material-" + "common color material", hoveredMat, true);
  ...
  this.init = function (scene) {
    ...
    _cursor.add(
      new THREE.Mesh(
        new THREE.RingGeometry(0.6, 1, 32, 32),
        new THREE.MeshBasicMaterial({
          color: DEFAULT_RED,
          depthTest: false,
          side: THREE.DoubleSide,
  ...
  this.update = function (camera, hud) {
    ...
    if (hit && curMode == modes.SelectMode) {
      ...
      hoveredObject = projectObjects.find((obj) => {
        return (
          obj.object3d.dbId == hit.dbId &&
          obj.object3d.fragId == hit.fragId &&
          obj.object3d.model === hit.model
        );
      });
      ...
      if (!hoveredObject) {
        ...
        projectObjects.push(
          new ProjectObject(
            hit,
            hit.model.getFragmentList().getMaterial(hit.fragId)
          )
        );
      ...
      if (!lastHovered) {
        ...
        if (
          hit.dbId !== lastHovered.object3d.dbId &&
          hit.fragId !== lastHovered.object3d.fragId
        ) {
          ...
          if (!lastHovered.selected) {
            lastHovered.object3d.model
              .getFragmentList()
              .setMaterial(lastHovered.object3d.fragId, lastHovered.material);
          }
  ...
  this.deselectEverything = function (menu) {
    projectObjects.forEach((element) => {
      element.object3d.model
        .getFragmentList()
        .setMaterial(element.object3d.fragId, element.material);
